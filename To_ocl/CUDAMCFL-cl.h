...
typedef struct
{
								cl_mem p; // Pointer to structure array containing all the photon data

								cl_mem x; // Pointer to the array containing all the WMC x's
								cl_mem a; // Pointer to the array containing all the WMC a's

								cl_mem thread_active; // Pointer to the array containing the thread active status

								cl_mem num_terminated_photons; //Pointer to a scalar keeping track of the number of terminated photons

								cl_mem Rd_xy; // Matriz 2D for reflexion
								cl_mem Tt_xy; // Matriz 2D for transmission
								cl_mem fhd; // 3D Matrix for the foton hitting density, ULL because 32-bit float does not have enough precision.

								cl_mem bulk_info; // 3D Matrix with a short integer per voxel selecting bulk composition

								cl_mem time_xyt; // 3D Matric with the temporal histogram in each detector

								cl_mem tdet_pos_x; // Array with the positions of the time detectors in x axis
								cl_mem tdet_pos_y; // Array with the positions of the time detectors in y axis
}MemStruct;
