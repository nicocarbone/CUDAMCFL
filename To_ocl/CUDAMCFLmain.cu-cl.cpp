...
#include <limits.h>
#include <stdio.h>
//#include "cuda_profiler_api.h-cl.h"

  cl_mem num_photons_dc[1];
  cl_mem n_layers_dc[1];
  cl_mem n_bulks_dc[1];
  cl_mem start_weight_dc[1];
  cl_mem layers_dc[MAX_LAYERS];
  cl_mem bulks_dc[MAX_LAYERS];
  cl_mem det_dc[1];
  cl_mem inclusion_dc[1];
  cl_mem ignoreAdetection_dc[1];
  cl_mem fhd_activated_dc[1];
  cl_mem do_temp_sim_dc[1];
  cl_mem bulk_method_dc[1];
  cl_mem xi_dc[1];
  cl_mem yi_dc[1];
  cl_mem zi_dc[1];
  cl_mem dir_dc[1];
  cl_mem esp_dc[1];
  cl_mem grid_size_dc[1];
  cl_mem max_temp_dc[1];

#include "CUDAMCFLio.cu-cl.cpp"
#include "CUDAMCFLmem.cu-cl.cpp"
#include "CUDAMCFLrng.cu-cl.cpp"
#include "CUDAMCFLtransport.cu-cl.cpp"
...
unsigned long long DoOneSimulation(SimulationStruct *simulation, unsigned long long *x,
  ...
  if (simulation->do_temp_sim == 1) {
    ...
    clEnqueueWriteBuffer(__cu2cl_CommandQueue, DeviceMem.tdet_pos_x, CL_TRUE, 0, num_x_tdet * sizeof(float), HostMem.tdet_pos_x, 0, NULL, NULL);
    clEnqueueWriteBuffer(__cu2cl_CommandQueue, DeviceMem.tdet_pos_y, CL_TRUE, 0, num_y_tdet * sizeof(float), HostMem.tdet_pos_y, 0, NULL, NULL);
  }

  size_t dimBlock[3] = {NUM_THREADS_PER_BLOCK, 1, 1};
  size_t dimGrid[3] = {NUM_BLOCKS, 1, 1};
  ...
  clFinish(__cu2cl_CommandQueue); // Wait for all threads to finish
...
/*CU2CL Unsupported -- Unsupported CUDA call: cudaGetErrorString*/
    ...
    clEnqueueReadBuffer(__cu2cl_CommandQueue, DeviceMem.thread_active, CL_TRUE, 0, NUM_THREADS * sizeof(unsigned int), HostMem.thread_active, 0, NULL, NULL);
    threads_active_total = 0;
    for (ii = 0; ii < NUM_THREADS; ii++)
      threads_active_total += HostMem.thread_active[ii];

    clEnqueueReadBuffer(__cu2cl_CommandQueue, DeviceMem.num_terminated_photons, CL_TRUE, 0, sizeof(unsigned long long), HostMem.num_terminated_photons, 0, NULL, NULL);
    if (i == 100)
