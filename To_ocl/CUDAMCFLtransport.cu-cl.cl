...
__kernel void MCd(MemStruct);
__kernel void MCd3D(MemStruct);
 float rand_MWC_oc(unsigned long long*,unsigned int*);
 float rand_MWC_co(unsigned long long*,unsigned int*);
 void LaunchPhoton(PhotonStruct*,unsigned long long*, unsigned int*, MemStruct);
__kernel void LaunchPhoton_Global(MemStruct);
 void Spin(PhotonStruct*, float,unsigned long long*,unsigned int*);
 unsigned int Reflect(PhotonStruct*, int, unsigned long long*, unsigned int*, int);
 unsigned int PhotonSurvive(PhotonStruct*, unsigned long long*, unsigned int*);
 unsigned int MoveToFirstBoundary(PhotonStruct*, unsigned short, short*, float);

__kernel void MCd(MemStruct DeviceMem)
{
  // Block index
  ...
  const float size_x = native_divide(det_dc[0].dx*__int2float_rn(det_dc[0].nx), 2.);
  const float size_y = native_divide(det_dc[0].dy*__int2float_rn(det_dc[0].ny), 2.);
  ...
  unsigned long long int x=DeviceMem.x[begin+tx];//coherent
    ...
    //Check for upwards reflection/transmission & calculate new s
      ...
      new_layer--;
      s = native_divide(layers_dc[p.layer].z_min-p.z, p.dz);
      }

    //Check for downward reflection/transmission
		if(p.z+s*p.dz>=layers_dc[p.layer].z_max){
      new_layer++;
      s = native_divide(layers_dc[p.layer].z_max-p.z, p.dz);
    }
    ...
    if (*fhd_activated_dc == 1){
      if(fabs(p.x)<2*esp && fabs(p.y)<2*esp && p.z<esp){ //Inside space of fhd
    ...
    // New layer?
      ...
      if(Reflect(&p,new_layer,&x,&a,1)==0u) {
        ...
        if(new_layer == 0){
          // Diffuse reflectance
          if(fabs(p.x-det_dc[0].x0)<size_x && fabs(p.y-det_dc[0].y0)<size_y) {
          ...
          if (*do_temp_sim_dc==1u && det_dc[0].temp_rort==0u && p.tof<det_dc[0].max_temp){
            ...
            for (int xpos = 0; xpos < num_x_tdet; xpos++){
              for (int ypos = 0; ypos < num_y_tdet; ypos++){
                if ((pow((p.x - DeviceMem.tdet_pos_x[xpos]), 2) + pow((p.y - DeviceMem.tdet_pos_y[ypos]), 2)) < pow((det_dc[0].temp_det_r), 2)){
	...
	barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);//necessary?
...
__kernel void MCd3D(MemStruct DeviceMem)
{
  ...
  //bool in_glass = FALSE;
    ...
    // Accumulate photon hitting density if needed
    if(fabs(p.x)<2*(*esp_dc) && fabs(p.y)<2*(*esp_dc) && p.z<(*esp_dc) && *fhd_activated_dc == 1) {
      // Inside space of 3D matrix. Calculate index of the current voxel
      // Use round to zero so there are no over sampled voxels (for ex: (max_x,0,0) and (0,1,0) should not map to the same voxel)
      index = __float2uint_rz((p.x+2*(*esp_dc))*__int2float_rn(*grid_size_dc))
            + num_x * (__float2uint_rz((p.y+2*(*esp_dc))*__int2float_rn(*grid_size_dc))
 ...
 void LaunchPhoton(PhotonStruct* p, unsigned long long* x, unsigned int* a, MemStruct DeviceMem)
{
  ...
  if (*dir_dc == 0.0f) {
    ...
    if ((*bulk_method_dc) == 2){
      if (p->z<(*esp_dc)){
        if(fabs(p->x)<2*(*esp_dc) && fabs(p->y)<2*(*esp_dc)){ //Inside space of fhd
          //Use round to zero so there are no over sampled voxels (for ex: (max_x,0,0) and (0,1,0) should not map to the same voxel)
    ...
    float sin_phi;
    float cos_phi;
    (*&sin_phi = sincos(sample_phi, &cos_phi));
...
__kernel void LaunchPhoton_Global(MemStruct DeviceMem)//PhotonStruct* pd, unsigned long long* x, unsigned int* a)
{
	int bx=get_group_id(0);
  int tx=get_local_id(0);
 ...
 void Spin(PhotonStruct* p, float g, unsigned long long* x, unsigned int* a)
{
	...
	temp = native_divide((1.0f-(g)*(g)), (1.0f-(g)+2.0f*(g)*rand_MWC_co(x,a)));// Should be close close????!!!!!
	cost = native_divide((1.0f+(g)*(g) - temp*temp), (2.0f*(g)));
	if(g==0.0f)
		cost = 2.0f*rand_MWC_co(x,a) -1.0f;// Should be close close??!!!!!

	sint = sqrt(1.0f - cost*cost);

	(*&cosp = sincos(2.0f*PI*rand_MWC_co(x,a), &sinp));// spin psi [0-2*PI)

	temp = sqrt(1.0f - p->dz*p->dz);
	...
	{
		p->dx = native_divide(sint*(p->dx*p->dz*cosp - p->dy*sinp), temp) + p->dx*cost;
		p->dy = native_divide(sint*(p->dy*p->dz*cosp + tempdir*sinp), temp) + p->dy*cost;
	...
	temp=rsqrt(p->dx*p->dx+p->dy*p->dy+p->dz*p->dz);
	p->dx = p->dx*temp;
	p->dy = p->dy*temp;
	p->dz = p->dz*temp;
}// end Spin
 ...
 unsigned int Reflect(PhotonStruct* p, int newlb, unsigned long long* x, unsigned int* a, int bulk_method)
{
  ...
  if (bulk_method==1){
	...
	float e = native_divide(n1*n1, n2*n2)*(1.0f-cos_angle_i*cos_angle_i); //e is the sin square of the transmission angle
	r=2*sqrt((1.0f-cos_angle_i*cos_angle_i)*(1.0f-e)*e*cos_angle_i*cos_angle_i);//use r as a temporary variable
	e=e+(cos_angle_i*cos_angle_i)*(1.0f-2.0f*e);//Update the value of e
	r = e*native_divide((1.0f-e-r), ((1.0f-e+r)*(e+r)));//Calculate r
 ...
 unsigned int MoveToFirstBoundary(PhotonStruct* p, unsigned short old_bulk, short* bulk_info, float max_s){
  ...
  while (present_bulk == old_bulk && total_move<max_s){
    ...
    if(p->z+p->dz*search_step<0.) {
      present_bulk = 0;
      index = -1;
      search_step = native_divide(-p->z, p->dz);
    }
    ...
    //Check for downward reflection/transmission
      ...
      index = -1;
      search_step = native_divide((*esp_dc)-p->z, p->dz);
    }

    else if (fabs(p->x+p->dx*search_step)>=2*(*esp_dc) || fabs(p->y+p->dy*search_step)>=2*(*esp_dc)){
      present_bulk=1;
      index = -1;
    }
 ...
 unsigned int PhotonSurvive(PhotonStruct* p, unsigned long long* x, unsigned int* a)
